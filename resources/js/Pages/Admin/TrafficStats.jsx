...
export default function TrafficStats() {
    ...
    const [filter, setFilter] = useState({ category_id: '', date_from: '', date_to: '' });
    ...
    const fetchStats = async () => {
        ...
        const res = await fetch('/admin/traffic-stats' + (params ? `?${params}` : ''));
    ...
    return (
        <div className="max-w-5xl mx-auto p-4">
            <h2 className="text-2xl font-bold mb-4">Statistik Traffic Berita (Admin)</h2>
            <div className="mb-4 flex gap-2">
                <input name="category_id" value={filter.category_id} onChange={handleChange} placeholder="ID Kategori" className="border p-2" />
            ...
            <table className="w-full border">
                ...
                <tbody>
                    {news.map(n => (
                        <tr key={n.id}>
                            <td>{n.title}</td>
                            <td>{n.categories && n.categories.map(c => c.name).join(', ')}</td>
                            <td>{n.user ? n.user.name : '-'}</td>
